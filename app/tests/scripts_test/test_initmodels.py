...
def test_init():
    ...
    assert Fleets_drivers_vehicles_rate.objects.count() == sum(
        [len(x["fleets_drivers_vehicles_rate"]) for x in DRIVERS_MAP["drivers"]]
    )
    assert DriverRateLevels.objects.count() == len(
        DRIVERS_MAP["driver_rate_levels"]
    )
