...
def test_bolt_payments_order(db) -> BoltPaymentsOrder:
    return BoltPaymentsOrder.objects.create(
        ...
        report_file_name=(
            'Щотижневий звіт Bolt – 2022W35 – Kyiv Fleet 03_232 park '
            'Universal-auto.csv'
        ),
...
def test_db(test_bolt_payments_order):
    ...
    assert test_bolt_payments_order.report_file_name == (
        'Щотижневий звіт Bolt – 2022W35 – Kyiv Fleet 03_232 park '
        'Universal-auto.csv'
    )
    ...
    assert test_bolt_payments_order.range_string == (
        "Тиждень 2022-08-29 - 2022-09-04"
    )
...
def test_total_cach_less_drivers_amount(test_bolt_payments_order):
    assert (
        test_bolt_payments_order.total_amount
        + test_bolt_payments_order.fee
        + test_bolt_payments_order.cancels_amount
        + test_bolt_payments_order.driver_bonus
        == 5424.75
    )
...
def test_total_drivers_amount(test_bolt_payments_order, rate=0.65):
    assert round(
        ...
        + test_bolt_payments_order.total_amount_cach,
        2
    ) == -403.91
