...
def test_uklon_payments_order(db) -> UklonPaymentsOrder:
    return UklonPaymentsOrder.objects.create(
        ...
        report_file_name=(
            "Куцко - Income_8_29_2022 3_00_00 AM-9_5_2022 3_00_00 AM.csv"
        ),
        signal="324460",
...
def test_db(test_uklon_payments_order):
    ...
    assert (
        test_uklon_payments_order.report_file_name
        == "Куцко - Income_8_29_2022 3_00_00 AM-9_5_2022 3_00_00 AM.csv"
    )
    ...
    assert (
        test_uklon_payments_order.total_amount_without_comission
        == float(1022.38)
    )
...
def test_total_drivers_amount(test_uklon_payments_order, rate=0.35):
    assert (
        -((test_uklon_payments_order.total_amount * 0.81) * rate)
        == -1704.969
    )
...
def test_type(test_uklon_payments_order):
    ...
    assert isinstance(
        test_uklon_payments_order.total_amount_without_comission, float
    )
